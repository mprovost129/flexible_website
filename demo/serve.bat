...
REM If something is already using port 8001, this will warn you first.

cd /d "%~dp0.."

echo Checking port 8001 is free...
netstat -ano | findstr ":8001 " | findstr "LISTENING" >nul 2>&1
if not errorlevel 1 (
    echo.
    echo ERROR: Something is already running on port 8001.
    echo.
...
echo.
echo [3/3] Starting server on http://127.0.0.1:8001/ ...
echo Keep this window open while recording.
echo.
python manage.py runserver 8001 --settings=config.Settings.demo
